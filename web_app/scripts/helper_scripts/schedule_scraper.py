...
DEFAULT_BASE_URL = "https://json.tab.co.nz/schedule"
DEFAULT_DAYS = 1825
DEFAULT_OUTFILE = "tab_schedule_master.ndjson"
...
def fetch_schedule_for_date(base_url: str, d: dt.date, timeout: int = 60) -> Optional[dict]:
    url = f"{base_url}/{d.isoformat()}"
    headers = {
        "Accept": "application/json",
        "User-Agent": "tab-schedule-scraper/1.0"
    }
...
def main():
    parser = argparse.ArgumentParser(description="Fetch TAB NZ schedule for the last N days and append to a master NDJSON file.")
    ...
    parser.add_argument("--base-url", type=str, default=DEFAULT_BASE_URL, help="Base URL for schedule endpoint")
    parser.add_argument("--sleep", type=float, default=0.5, help="Seconds to sleep between requests")
    parser.add_argument("--force", action="store_true", help="Ignore existing dates and re-fetch")
    ...
    for d in daterange(end_date, want_days):
        ds = d.isoformat()
        if not args.force and ds in existing:
            skipped += 1
            print(f"Skip {ds} already in file")
            continue
        try:
            payload = fetch_schedule_for_date(args.base_url, d)
            if payload is None:
                print(f"No schedule for {ds} (404)")
                continue
